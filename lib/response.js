...
const headings = Headings.load(readFileSync(TODO_STORE, 'utf8'));

const saveList = function(req, res) {
  const { id, title } = req.body;
  headings.addNewTodo(id, title);
  saveInFile();
  res.statusCode = 201;
...
const loadPreviousTodo = function(req, res) {
  res.setHeader('content-type', 'application/json');
  res.end(headings.todo(`${req.body.taskId}`).toStringify());
};
